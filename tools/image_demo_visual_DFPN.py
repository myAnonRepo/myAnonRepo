...
def main():
    ...
    config = '/home/maer/Workspace/mmdetection/configs/coco/faster_rcnn_r50_fpn_1x_coco_fpnd22.py'
    checkpoint = '/home/maer/Workspace/mmdetection/work_dirs/faster_rcnn_r50_fpn_1x_coco_fpn_d22/latest.pth'
    device = 'cuda:0'
    ...
    for i in os.listdir(images_dir):
        ...
        result, x_backbone, laterals_copy, laterals_td_sum_copy, interouts_copy, interouts_bu_sum_copy, x_fpn = inference_detector(model, img_path)
        ...
        for feature in x_backbone:
            ...
            for c in c_list:
                ...
                heatmap_image = np.uint8(heatmap)
        ...
        feature_index = 1
        for feature in laterals_td_sum_copy:
            feature_index += 1
        ...
        feature_index = 1
        for feature in interouts_copy:
            feature_index += 1
            IO_save_path = os.path.join(fm_save_path, 'IO_'+str(feature_index))
            ...
            for c in range(C):
                ...
                cv2.imwrite(IO_save_path + '/'+ str(c)+'_heatmap.jpg', heatmap_image)
                result = cv2.addWeighted(image, 1, heatmap_image, 0.5, 0)
                cv2.imwrite(IO_save_path + '/'+ str(c) + '_result.jpg', result)

        feature_index = 1
        for feature in interouts_bu_sum_copy:
            feature_index += 1
            IOS_save_path = os.path.join(fm_save_path, 'IOS_'+str(feature_index))
            if not os.path.exists(IOS_save_path):
                os.makedirs(IOS_save_path)
            P = torch.sigmoid(feature)
            #P = feature
            P = P.cpu().detach().numpy()
            P = np.maximum(P, 0)
            ...
            for c in range(C):
                ...
                cv2.imwrite(IOS_save_path + '/'+ str(c)+'_heatmap.jpg', heatmap_image)
                result = cv2.addWeighted(image, 1, heatmap_image, 0.5, 0)
                cv2.imwrite(IOS_save_path + '/'+ str(c) + '_result.jpg', result)
